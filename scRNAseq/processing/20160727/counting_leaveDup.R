...
require(Rsubread)
out <- featureCounts(bam.files, annot.ext="temp.gtf", isGTFAnnotationFile=TRUE, minMQS=10, nthreads=4, isPairedEnd=ispet, strandSpecific=strandspec, ignoreDup=FALSE)
...
write.table(file="genic_counts_leaveDup.tsv", final, col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
