...
alt.fit <- trendVar(sceunind, use.spikes=TRUE, design=mod, method='loess', parametric='true', span=0.6) 
...
points(alt.decomp$mean[which(isSpike(sceunind))], alt.decomp$total[which(isSpike(sceunind))], col='red', pch=20)
...
Top gene Dusp1 is a Mapk phosphatase - this could be interesting with respect to signalling initiation.

Adhesion molecule Itgb2 is CD18, part of LFA-1, which is known to play a role in T cell activation.

Cd6 is known to have a role in T cell costimulation

Jun is part of AP-1 transcription factors.
...
Finally, let's make the same heatmap but instead colour the side-bars by pseudotime and batch to get more of an overview.
