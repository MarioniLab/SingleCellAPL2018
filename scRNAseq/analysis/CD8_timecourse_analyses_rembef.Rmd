...
This script runs through analyses of the timecourse data from a single-cell sequencing experiment looking at early activation of CD8+ T cells - this data was collected from an experiment run on 261016. Here we are using the removeBatchEffect function from limma instead of ComBat to test whether there are any differences.
...
rbe_sce <- removeBatchEffect(x=exprs(sce), batch=sce$Sorting.Plate,
                     design=mod)
exprs(sce) <- rbe_sce
...
Now, let's look at the most activated cluster (cluster 3) - how does it compare to unstimulated cells?
...
So let's look at gene set enrichment in cells in the early activation versus unstimulated and late activation versus unstimulated clusters.  We note that we are taking into account mean gene expression (a proxy for gene length) in this analysis as it is comparative between gene sets.
...
GO1 <- goana(res1, species="Mm", geneid=xx[rownames(res1)], FDR=0.05, trend=TRUE)
GO1tab <- topGO(GO1, ont="BP", number=Inf)
head(GO1tab)

GO2 <- goana(res2, species="Mm", geneid=xx[rownames(res2)], FDR=0.05, trend=TRUE)
