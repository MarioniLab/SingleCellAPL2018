...
In contrast to the original file, this uses removeBatchEffect from limma instead of ComBat from sva to test whether there is a difference.
...
rbe_sce <- removeBatchEffect(x=exprs(sce2), batch=sce2$Sorting.Plate,
                     design=mod)
exprs(sce2) <- rbe_sce
```
...
rbe_sce <- removeBatchEffect(x=exprs(sce1), batch=sce1$Sorting.Plate,
                     design=mod)
exprs(sce1) <- rbe_sce
```
...
# dev.off()
# 
# pdf(paste0('data/CD69_protein_APL2_spikenorm_filt05_violinplot.pdf'), height=2.5, width=2)
# protein_violin_plot('CD69', sceset=sce2, 
...
# dev.off()
# 
# pdf(paste0('data/CD25_protein_APL2_spikenorm_filt05_violinplot.pdf'), height=2.5, width=2)
# protein_violin_plot('CD25', sceset=sce2, 
#             split_facs=c('N4 6h', 'T4 6h', 'G4 6h', 'NP68 6h'), 
#             cols=c('red', 'green3', 'blue', 'grey40'))
# dev.off()
